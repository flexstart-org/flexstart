...
export default function Contact() {
  ...
  const { firstname, lastname, company, email, number, message } = data;
    ...
    <div className="px-6 py-8 bg-white isolate sm:py-8 lg:px-8">
      ...
      <form
        onSubmit={async (e) => {
          e.preventDefault();
          setSaving(true);
          fetch(`https://dash.flexstart.org/api/email`, {
            method: "POST",
            headers: {
      ...
      >
        ...
        <div className="mt-10">
          <button
            ...
            } block w-full rounded-md px-3.5 py-2.5 text-center text-sm font-semibold shadow-sm focus-visible:outline focus-visible:outline-offset-2 focus-visible:outline-indigo-600`}
          >
            {saving ? <LoadingDots color="#808080" /> : "Submit"}
          </button>
        </div>
      </form>
    </div>
