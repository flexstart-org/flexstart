...
In the `app` directory, [nested folder hierarchy](/courses/nextjs/routing#route-segments) defines route structure.
...
```js filename="app/dashboard/settings/analytics/page.js" showLineNumbers
// before
...
<FileTree>
  In our examples below, we're using `components` and `lib` folders as
  generalized placeholders, their naming has no special framework significance
  and your projects might use other folders like `ui`, `utils`, `hooks`,
  `styles`, etc.
</FileTree>
