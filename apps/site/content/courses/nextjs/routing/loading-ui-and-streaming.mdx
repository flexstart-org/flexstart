...
```tsx filename="app/dashboard/loading.tsx" showLineNumbers
...
- Next.js will wait for data fetching inside [`generateMetadata`](/learn/nextjs/functions/generate-metadata) to complete before streaming UI to the client. This guarantees the first part of a streamed response includes `<head>` tags.
