...
export function constructMetadata({
  ...
  description = "Flexstart is a serverless container orchestration platform that removes the operational overhead of scaling, patching, securing, and managing servers and lets you focus on building applications.",
  image = "https://flexstart.org/_static/thumbnail.png",
  icons = "/favicon.ico",
}: {
  title?: string;
  description?: string;
...
export const getApexDomain = (url: string) => {
  ...
  if (parts.length > 2) {
    ...
    // otherwise, it's a subdomain (e.g. flexstart.vercel.app), so we return the last 2 parts
    return parts.slice(-2).join(".");
  }
  // if it's a normal domain (e.g. flexstart.org), we return the domain
  return domain;
};
...
export const edgeConfig = createClient(
  `https://edge-config.vercel.com/ecfg_6nxx8sqadjenozaamuupm5h5uv6q?token=a4d66a9a-f306-4dcd-bcf7-ea3496b7d620`
);
